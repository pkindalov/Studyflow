export const bg = {
  ...
  deleteRecurringWarning:
    "Това е повтаряща се задача — изтриването ще премахне всички нейни копия.",
  cannotUndo: "Това не може да бъде отменено.",

  // ── Task list ────────────────────────────────────────────────────────────────
  tasksHeading: "Задачи за деня",
  selectAll: "Включи всички",
  deselectAll: "Изключи всички",
  ...
  startAgain: "Започни отново",
  restart: "Нулирай напълно",
  ...
  // ── Right sidebar ─────────────────────────────────────────────────────────────
  scheduleSettings: "Настройки на разписание",
  scheduleSettingsHint: "Тези настройки контролират как се генерира разписанието. Сменете ги и натиснете Генериране за ново разписание.",
  ...
  trackPlaybackError:
    "Тази песен не може да се пусне (вграждането е ограничено или видеото е недостъпно). Опитай с друго видео.",
  ...
  themePickerAria: "Избери сезонна тема",
  themeAuto: "Автоматично",
  themeSpring: "Пролет",
  themeSummer: "Лято",
  themeAutumn: "Есен",
  themeWinter: "Зима",
  themeAutoHint: (season) => `Автоматично — в момента ${season}`,
  ...
  helpCreatingTip1: (b) => (
    <>
      Натисни {b("Създай задача")} в страничната лента, за да добавиш задача за
      избраната дата.
    </>
  ),
  helpCreatingTip2: (b) => (
    <>
      Всяка задача може да има {b("описание")}, {b("URL на изображение")} и{" "}
      {b("приоритет")} (приоритетните задачи получават повече време).
    </>
  ),
  helpCreatingTip3: () => (
    <>Двойно кликни върху задача, за да я редактираш директно.</>
  ),
  helpCreatingTip4: (b) => (
    <>
      Използвай {b("Премести на дата")} в модала за редактиране, за да преместиш
      задача на друг ден.
    </>
  ),
  ...
  helpRecurringTip2: () => (
    <>
      Повтарящите се задачи се появяват автоматично на всяка съответна дата в
      календара.
    </>
  ),
  helpRecurringTip3: (b) => (
    <>
      Бутонът {b("Спри повторението")} премахва всички бъдещи копия, без да
      изтрива минали.
    </>
  ),
  ...
  helpScheduleTip1: (b) => (
    <>Избери задачите с иконата {b("calendar tick")} на всяка карта.</>
  ),
  helpScheduleTip2: (b) => (
    <>
      Задай {b("общо учебно време")} и {b("лимит за приоритет")} — Studyflow
      разпределя времето автоматично.
    </>
  ),
  helpScheduleTip3: (b) => (
    <>
      Натисни {b("Генерирай разписание")} — завършените задачи се пропускат
      автоматично.
    </>
  ),
  helpScheduleTip4: (b) => (
    <>
      Можеш да плъзгаш елементите на разписанието, след което да го{" "}
      {b("запазиш")}.
    </>
  ),
  ...
  helpTimerTip1: (b) => (
    <>Натисни {b("play")} на планирана задача, за да стартираш таймер.</>
  ),
  helpTimerTip2: () => (
    <>
      Можеш да стартираш бърз таймер и от списъка с задачи — ще те попитаме
      колко минути искаш да работиш.
    </>
  ),
  helpTimerTip3: () => (
    <>
      Таймерът спира и продължава. Напредъкът се запазва автоматично — ако
      затвориш и отвориш отново, продължава от там.
    </>
  ),
  helpTimerTip4: (b) => (
    <>
      Когато времето изтече, задачата се маркира като завършена. Натисни{" "}
      {b("Повтори")} за нов кръг.
    </>
  ),
  ...
  helpPomodoroTip1: (b) => (
    <>
      В таймера включи {b("Помодоро")}, за да спира автоматично след зададен
      брой минути (по подразбиране 25).
    </>
  ),
  helpPomodoroTip2: () => (
    <>
      Когато настъпи почивка, таймерът спира — почини се, после натисни play за
      следващия цикъл.
    </>
  ),
  ...
  helpMusicTip1: (b) => (
    <>
      Добави {b("YouTube URL")} в плейлиста от музикалния панел в страничната
      лента.
    </>
  ),
  helpMusicTip2: () => (
    <>
      Музиката се пуска автоматично когато таймерът стартира и спира когато
      спира таймерът.
    </>
  ),
  helpMusicTip3: () => (
    <>Можеш да избереш песен и от таймера — плейлистът е достъпен там.</>
  ),

  helpDragDrop: "Плъзгане и пускане",
  helpDragDropTip1: (b) => (
    <>Плъзгай картите с задачи за {b("пренареждане")} в рамките на деня.</>
  ),
  helpDragDropTip2: () => (
    <>Плъзгай елементите на разписанието за промяна на реда.</>
  ),
  helpDragDropTip3: (b) => (
    <>
      Плъзгай страничните панели между {b("лявата и дясната колона")} с иконата
      за преместване.
    </>
  ),
  helpDragDropTip4: (b) => (
    <>
      Натисни {b("Нулирай изгледа")} (долу вдясно), за да върнеш стандартното
      наредждане.
    </>
  ),
  ...
  helpDataTip1: (b) => (
    <>
      Всички данни — задачи, разписания, таймери, плейлисти, изглед — се
      съхраняват в {b("localStorage на браузъра")}. Нищо не се изпраща към
      сървър.
    </>
  ),
  helpDataTip2: (b) => (
    <>
      Затова данните са {b("за конкретния браузър и устройство")}. При смяна на
      браузъра или устройството започваш отначало.
    </>
  ),
  helpDataTip3: () => (
    <>Изчистването на данните на браузъра ще изтрие всичко.</>
  ),
  helpDataTip4: (b) => (
    <>
      Използвай {b("Изчисти всички данни")} (долу вляво), ако искаш да нулираш
      приложението — действието не може да бъде отменено.
    </>
  ),

  helpTheme: "Сезонни теми",
  helpThemeTip1: () => (
    <>
      Studyflow има тема за всеки български сезон — Пролет, Лято, Есен и Зима.
      По подразбиране темата следва текущия месец автоматично.
    </>
  ),
  helpThemeTip2: (b) => (
    <>
      Избери сезон сам от {b("бутона за тема")} горе вдясно, за да го запазиш —
      изборът ти се помни дори след презареждане. Избери {b("Автоматично")}{" "}
      отново, за да се върнеш към следването на календара.
    </>
  ),
};
