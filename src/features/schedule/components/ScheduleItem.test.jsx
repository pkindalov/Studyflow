...
describe('rendering', () => {
  it('renders task text', () => {
    render(<ScheduleItem task={base} elapsed={0} isRunning={false} runningTaskId={null} onOpenTimer={onOpenTimer} onMarkDone={onMarkDone} onRemove={onRemove} t={t} />)
  ...
  it('shows priority badge when task.priority is true', () => {
    render(<ScheduleItem task={{ ...base, priority: true }} elapsed={0} isRunning={false} runningTaskId={null} onOpenTimer={onOpenTimer} onMarkDone={onMarkDone} onRemove={onRemove} t={t} />)
...
describe('timer button title', () => {
  ...
  it('shows "Completed" title when task is done', () => {
    render(<ScheduleItem task={{ ...base, done: true }} elapsed={0} isRunning={false} runningTaskId={null} onOpenTimer={onOpenTimer} onMarkDone={onMarkDone} onRemove={onRemove} t={t} />)
    expect(screen.getByTitle('Completed')).toBeTruthy()
  })

  it('shows "Resume timer" title when elapsed > 0 and not finished', () => {
    render(<ScheduleItem task={base} elapsed={300} isRunning={false} runningTaskId={null} onOpenTimer={onOpenTimer} onMarkDone={onMarkDone} onRemove={onRemove} t={t} />)
...
describe('mark-done button visibility', () => {
  ...
  it('hides mark-done button when elapsed reaches total', () => {
    render(<ScheduleItem task={base} elapsed={1800} isRunning={false} runningTaskId={null} onOpenTimer={onOpenTimer} onMarkDone={onMarkDone} onRemove={onRemove} t={t} />)
    expect(screen.queryByTitle('Mark as Done')).toBeNull()
  })
})
...
describe('progress bar', () => {
  it('shows progress bar when elapsed > 0', () => {
    const { container } = render(<ScheduleItem task={base} elapsed={600} isRunning={false} runningTaskId={null} onOpenTimer={onOpenTimer} onMarkDone={onMarkDone} onRemove={onRemove} t={t} />)
