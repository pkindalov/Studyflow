...
const TaskBankListTab = function({ taskBank, selectedIds, onToggle, onAddToBank, onRemoveFromBank, onUpdateInBank, onReorderBank }) {
  ...
  const saveEdit = function() {
    if (editText.trim() && editingId) onUpdateInBank(editingId, editText.trim(), editPriority);
    setEditingId(null);
  };
        ...
        editingId === task.id ? (
          <div key={task.id} className="flex items-center gap-2 p-2 rounded-xl bg-surface-container-highest border border-primary/40">
            ...
            <button
              ...
              className={`p-1.5 rounded-lg border transition-all flex-shrink-0 ${editPriority ? "bg-tertiary/15 border-tertiary/40 text-tertiary" : "border-outline/40 text-on-surface-variant hover:text-tertiary"}`}
            >
            ...
            <button onClick={() => setEditingId(null)} className="p-1.5 rounded-lg text-on-surface-variant hover:bg-surface-container-high transition-colors flex-shrink-0">
              <span className="material-symbols-outlined text-base">close</span>
            </button>
          </div>
        ) : (
          <label
            ...
            onDrop={(e) => { e.preventDefault(); onReorderBank(e.dataTransfer.getData("text/plain"), task.id); setDragOverId(null); }}
          ...
          >
            ...
            <input type="checkbox" checked={selectedIds.has(task.id)} onChange={() => onToggle(task.id)} className="w-4 h-4 accent-primary flex-shrink-0" />
            <span className={`flex-1 text-sm text-on-surface min-w-0 ${task.priority ? "font-semibold" : "font-medium"}`}>{task.text}</span>
            {task.priority && (
              <span className="text-[10px] font-bold uppercase tracking-wider text-tertiary flex-shrink-0">{t.priorityBadge}</span>
            )}
            <button
      ...
      <form onSubmit={handleAddToList} className="flex flex-col gap-2 pt-2 pb-1 border-t border-outline-variant/30 mt-1">
        ...
        <div className="flex justify-end gap-2">
          <button
            ...
            title={t.priorityTaskLabel}
            className={`p-2 rounded-xl border transition-all ${newTaskPriority ? "bg-tertiary/15 border-tertiary/40 text-tertiary" : "border-outline/40 text-on-surface-variant hover:border-tertiary/40 hover:text-tertiary"}`}
          >
